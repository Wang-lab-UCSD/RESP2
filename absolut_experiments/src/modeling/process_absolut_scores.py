...
def check_absolut_scores(project_dir):
    ...
    # Next, check single-target success rates for those methods which
    # can be treated as single target.
    print("\n**************************************************\n"
            "Single target\n*****************************************")
    configurations = ["hyson_pipeline", "single_target_runs",
            "hle", "protein_mpnn"]

    for target_group, target_list in constants.TARGET_PROTEIN_GROUPS.items():
        for protein_code in target_list:
            for configuration in configurations:
                output_scores = {}
                bad_file = False
                ...
                # We fix this here.
                for seq, score in zip(seqs, scores):
                    output_scores[seq] = score

                score_fpath = os.path.join(raw_datapath,
                    f"{protein_code}_500kNonMascotte.txt")
                training_set_scores = read_raw_data_files([score_fpath])[1]
                ...
                output_scores = np.stack([s for _, s in output_scores.items()]).flatten()
                training_set_scores = np.array(training_set_scores)
                ...
                # 4) bootstrapped 95% CI on the success rate.
                rng = np.random.default_rng(123)
                full_success = get_single_target_success_rate(training_set_scores,
                                                             output_scores)
                ...
                for _ in range(1000):
                    idx = rng.choice(output_scores.shape[0],
                                     size = output_scores.shape[0],
                                     replace = True)
                    full = get_single_target_success_rate(training_set_scores,
                                                 output_scores[idx])
                    bootstrapped_full.append(full)

                bootstrapped_full = np.sort(bootstrapped_full)
                print(f"\nFor {protein_code}, {configuration}:\n"
                    f"SUCCESS RATE: {full_success}  "
                    f"{bootstrapped_full[25]} to {bootstrapped_full[975]}\n"
                    f"Generated {output_scores.shape[0]} candidates")
...
def get_single_target_success_rate(training_scores, output_scores):
    """Calculates the success rate for the output scores and training
    scores that are provided when there is only one target."""
    successes = output_scores < training_scores.min()
    return np.round(successes.sum() / successes.shape[0], 2)
