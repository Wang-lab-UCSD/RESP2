...
class bayes_regression_nn(torch.nn.Module):
    ...
    def __init__(self, sigma1_prior = 1.0, xdim=264,
        ...
        self.register_buffer("train_ymean", torch.zeros((1))  )
        self.register_buffer("train_ystd", torch.ones((1))  )

        self.activate_scaling = False


    def set_yscaling_factors(self, file_list):
        """Determines the mean and standard deviation of the y-data.
        If this is used, it must be called before training. The y-values
        in general take up sufficiently little memory that we can load
        them all at once."""
        yvalues = np.concatenate([np.load(yfile) for yfile in file_list])
        yvalues = torch.from_numpy(yvalues).float()
        self.train_ymean[0] = torch.mean(yvalues)
        self.train_ystd[0] = torch.std(yvalues)
    ...
    def train_model(self, xfiles, yfiles, valid_xfiles = None, valid_yfiles = None,
        ...
        self.set_yscaling_factors(yfiles)
        ...
        for epoch in range(0, epochs):
            ...
            for j in permutation:
                ...
                for k in range(0, x.shape[0], minibatch):
                    ...
                    y_mini = (y_mini - self.train_ymean) / self.train_ystd
    ...
    def map_predict(self, x):
        ...
        with torch.no_grad():
            self.eval()
            x = self.scale_data(x)
            preds = self.forward(x, sample=False)[0].flatten()
            preds = preds * self.train_ystd[0] + self.train_ymean[0]
            return preds.flatten()


    def predict(self, x, get_var = True, num_samples=50, random_seed=123):
        ...
        Args:
            ...
            get_var (bool): Not used; retained for consistency with other models
        ...
        Returns:
            ...
            std_scores (tensor): The uncertainty for each datapoint.
        """
        ...
        with torch.no_grad():
            ...
            scores = scores * self.train_ystd + self.train_ymean
            mean_scores, std_scores = torch.mean(scores, dim=-1), torch.std(scores, dim=-1)
            return mean_scores.cpu().numpy(), (std_scores.cpu().numpy() / np.sqrt(num_samples))**2
