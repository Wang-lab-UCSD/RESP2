...
def encode_batches(tt_split, label, stored_encodings, variant_dict,
    ...
    for i in range(0, len(stored_encodings["x"]), batch_size):
        ...
        x_conv = onehot_converter.conv1d_x_feat_extract(x_onehot, chunk_size=2000)
        a_conv = onehot_converter.conv1d_x_feat_extract(a_onehot_full, chunk_size=2000)
        ...
        a_lengths = np.array([a_pfa.shape[1] for _ in range(a_pfa.shape[0])],
                             dtype=np.int32)
        x_conv = onehot_converter.conv1d_x_feat_extract(x_pfa, chunk_size=2000)
        a_conv = onehot_converter.conv1d_x_feat_extract(a_pfa, chunk_size=2000)
        ...
        x_lengths = np.array([x_autoencoder.shape[1] for _ in range(x_autoencoder.shape[0])],
                             dtype=np.int32)
        x_conv = autoenc_converter.conv1d_x_feat_extract(x_autoencoder, chunk_size=2000)
